...
class SimpleGoBoard(object):
    ...
    def is_legal(self, point, color):
        ...
        legal = board_copy.play_move(point, color,2)
        return legal
    ...
    def play_move(self, point, color, play_operation):
        """
        Play a move of color on point

        play_operation used for the function of play_cmd, and generate_gogui_legal_moves which is 
        created for gogui_rules_legal_moves_cmd, 'play' for move and 'not play' for not move and 
        not change opponent

        we use number to represent the order of error
        -1 for pass error 
        -4 for occupied
        -5 for capture
        -6 for suicide
        note: -2 -3 is missed, and we handle in the play_cmd 
        where -2 is for wrong color and -3 is for wrong coordiate

        1 is for legal input which means no occupied,capture or suicide and pass
        """
        assert is_black_white(color)
        #pass 
        if point == PASS:
        ...
        #ko-recapture(sub-type of capture)
        ...
        for nb in neighbors:
            if self.board[nb] == opp_color:
                capture_int = self._detect_and_process_capture(nb)
                #if is captured
                if capture_int== -2: 
                    #undo capture move
        ...
        if play_operation == 'play': 
            self.current_player = GoBoardUtil.opponent(color)
        #to make the stone not move
        else: 
             self.board[point] = EMPTY
